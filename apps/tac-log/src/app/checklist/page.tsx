...
export default async function ChecklistPage({ searchParams }: { searchParams: Promise<{ list?: string; new?: string }> }) {
  ...
  const list = wanted && all.includes(wanted) ? wanted : extra[0] ?? all[0] ?? "Range Bag";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">{list} Checklist</h1>
          <p className="no-print text-sm text-neutral-400">Tick items off as you pack. Uncheck All resets it for the next trip.</p>
        </div>
        <div className="no-print flex flex-wrap items-center gap-2">
          {all.map((n) => (
            <Link
              ...
              className={`border px-3 py-1.5 text-xs ${n === list ? "border-brand-olive bg-brand-olive" : "border-neutral-700 text-neutral-400 hover:text-neutral-100"}`}
            >
              {n}
            </Link>
          ))}

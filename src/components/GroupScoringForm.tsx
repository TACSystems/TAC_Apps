...
import type { CofScoringZone, Firearm, Participant } from "@/lib/db/types";
import SubmitButton from "@/components/SubmitButton";

export default function GroupScoringForm({
  zones,
  firearms,
  participants,
  totalRounds,
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <label className="flex flex-col gap-1 text-sm">
          Firearm From Your Armory (optional)
          <select name="firearm_id" className="rounded border border-neutral-700 bg-neutral-900 px-3 py-2">
            <option value="">— None / their own —</option>
            {firearms.map((f) => (
        ...
        <label className="flex flex-col gap-1 text-sm">
          Weapon Used (general description)
          <input
            name="weapon_used"
            placeholder="e.g. AR-15"
            className="rounded border border-neutral-700 bg-neutral-900 px-3 py-2"
          />
        </label>
        <label className="flex flex-col gap-1 text-sm">
          Caliber
          <input name="caliber" className="rounded border border-neutral-700 bg-neutral-900 px-3 py-2" />
        </label>
        <label className="flex flex-col gap-1 text-sm">
          Range Location
          <input

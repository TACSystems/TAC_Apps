...
import type { ChecklistItem } from "@core/lib/checklist";

const input = "input input-sm";

export type ChecklistActions = {
  toggleItem: (id: string, checked: boolean) => Promise<void>;
  addItem: (list: string, section: string, text: string) => Promise<void>;
  deleteItem: (id: string) => Promise<void>;
  moveItem: (list: string, id: string, dir: -1 | 1) => Promise<void>;
  resetList: (list: string) => Promise<void>;
  renameList: (from: string, to: string) => Promise<void>;
  deleteList: (list: string) => Promise<void>;
};

export default function Checklist({ list, items, sections, actions }: { list: string; items: ChecklistItem[]; sections: string[]; actions: ChecklistActions }) {
  const { addItem, deleteItem, deleteList, moveItem, renameList, resetList, toggleItem } = actions;

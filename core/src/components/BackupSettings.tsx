...
export type BackupActions = {
  setBackupPassword: (password: string, confirm: string) => Promise<Res>;
  clearBackupPassword: () => Promise<Res>;
  saveAutoBackupSettings: (frequency: string, folder: string, keep: number) => Promise<Res>;
  backupNow: () => Promise<Res>;
};
...
export default function BackupSettings({
  ...
  const { setBackupPassword, clearBackupPassword, saveAutoBackupSettings, backupNow } = actions;
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-neutral-500">
            {hasPassword
              ? `Encrypted with your backup password (.${backupExt}).`
              : "Plain .zip. Set a backup password below to encrypt it."}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2 border border-neutral-800 p-3">
        ...
        <p className="text-xs text-neutral-500">
          ...
          records. You&apos;ll need this password to restore. It is separate from your unlock password.
      ...
      <div className="flex flex-col gap-3 border border-neutral-800 p-3">
        ...
        <p className="text-xs text-neutral-500">
          {productName} writes a full backup to the folder you pick (a USB drive, or a synced folder like OneDrive, iCloud Drive,
          or Dropbox) and keeps the most recent copies. If a backup was missed while the app was closed, it runs the next
          time {productName} opens.
        </p>

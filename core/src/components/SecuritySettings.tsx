...
export type SecurityActions = {
  savePin: (current: string, next: string, confirm: string) => Promise<Res>;
  removePin: (current: string) => Promise<Res>;
  enableEncryption: (current: string, password: string, confirm: string) => Promise<Res>;
  changePassword: (current: string, next: string, confirm: string) => Promise<Res>;
  newRecoveryKey: (current: string) => Promise<Res>;
  disableEncryption: (current: string) => Promise<Res>;
  saveAutoLock: (minutes: number) => Promise<unknown>;
};
...
export default function SecuritySettings({
  ...
  const { savePin, removePin, enableEncryption, changePassword, newRecoveryKey, disableEncryption, saveAutoLock } =
    actions;
  ...
  const status =
    mode === "password"
      ? `Database encryption is ON. Your data file and photos are encrypted, and ${productName} asks for your password every time it opens.`
      : mode === "pin"
        ? `A PIN is set. ${productName} asks for it every time it opens. The data file itself is not encrypted.`
        : `${productName} opens without a PIN or password.`;
  ...
  return (
    <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {mode === "pin" && (
                  <button
                    ...
                    disabled={pending}
                    onClick={() => {
                      ask(`Remove the PIN? ${productName} will open without asking.`).then((ok) => ok && run(() => removePin(current)));
                    }}
                    className={danger}
                  >
                    Remove PIN
        ...
        {panel === "encrypt" && (
          <>
            <p className="text-xs text-neutral-400">
              Before switching, {productName} checks the encrypted copy and puts things back if anything goes wrong. Backups
      ...
      {mode !== "none" && (
        <p className="text-xs text-neutral-500">
          {productName} also locks when the computer sleeps or the screen locks. After 5 wrong tries it makes you wait, starting
      ...
      {mode === "pin" && (
        <p className="text-xs text-neutral-500">
          Forgot your PIN? Close {productName}, create an empty file named RESET-PIN in the data folder shown under
          About below, then reopen {productName}. The PIN is cleared and nothing else changes. (This does not work for an encryption

...
import { closeDb, dataDir, getDb, rekeyFile } from "@core/lib/db-core";
import { createZip, readZip, type ZipEntry } from "@core/lib/zip";
import {
  dataKey,
  deriveKey,
  isEncrypted,
...
export function backupZip(db: Database.Database): { buffer: Buffer; receiptCount: number } {
  ...
  const buffer = createZip([
    ...
    { name: appProfile().dbFile, data: Buffer.from(snapshot) },
    ...receipts,
  ]);
...
function validateDatabase(file: string) {
  ...
  try {
    ...
    if (!appProfile().requiredTables.every((t) => tables.has(t))) {
      throw new Error(`That file isn't a ${appProfile().productName} database.`);
    }
...
export function restoreBackup(
  ...
  if (buf.subarray(0, 4).toString("latin1") === "PK\u0003\u0004") {
    ...
    const dbEntry = entries.find((e) => e.name === appProfile().dbFile);
    if (!dbEntry) throw new Error(`That zip doesn't contain a ${appProfile().productName} database.`);
    ...
    throw new Error(`Choose a ${appProfile().productName} backup (.zip) or database (.db) file.`);
  }
  ...
  closeDb();
  const live = path.join(dir, appProfile().dbFile);
  if (isEncrypted()) {
    const key = dataKey();
    if (!key) throw new Error(`${appProfile().productName} is locked.`);
    rekeyFile(tmp, null, key);
  }
  if (fs.existsSync(live)) fs.renameSync(live, path.join(safety, appProfile().dbFile));

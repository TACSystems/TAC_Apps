...
import { closeDb, dataDir, dbPath, getDb, rekeyFile } from "@core/lib/db-core";
import { sealAllAttachments, sealTree } from "@core/lib/backup";
import {
...
export { isUnlocked, securityMode, isEncrypted, lockoutState } from "@core/lib/security-state";
...
function safetyCopies() {
  ...
  if (fs.existsSync(restore)) {
    for (const d of fs.readdirSync(restore)) {
      const f = path.join(restore, d, appProfile().dbFile);
...
export function setPin(current: string, next: string, confirm: string): AuthResult {
  const sec = readSecurity();
  if (sec.mode === "password") return { ok: false, error: `Database encryption is on, so ${appProfile().productName} uses a password instead of a PIN.` };
  if (sec.mode === "pin") {
  ...
  setUnlocked();
  return { ok: true, message: `PIN saved. ${appProfile().productName} will ask for it each time it opens.` };
}
...
export function enableEncryption(current: string, password: string, confirm: string): AuthResult {
  ...
  if (!isUnlocked()) return { ok: false, error: `Unlock ${appProfile().productName} first.` };
  if (sec.mode === "pin") {
    const v = verifyCredential(current);
    if (!v.ok) return v;
  }
